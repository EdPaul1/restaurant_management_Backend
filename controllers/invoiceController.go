...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("error occurred while fetching order: %v", err)
		...
		if insertErr != nil {
			msg := fmt.Sprintf("error occurred while creating invoice: %v", insertErr)
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("error occurred while updating invoice: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
